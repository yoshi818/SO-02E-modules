...
#include "symsearch.h"
SYMSEARCH_DECLARE_FUNCTION_STATIC(int, sched_setscheduler_nocheck_s, struct task_struct *, int policy, const struct sched_param *);
...
static int __init cpufreq_intelliactive_init(void)
{
	...
	SYMSEARCH_BIND_FUNCTION_TO(intellilactive, sched_setscheduler_nocheck, sched_setscheduler_nocheck_s);
	...
	sched_setscheduler_nocheck_s(speedchange_task, SCHED_FIFO, &param);
