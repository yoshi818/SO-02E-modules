	...
	for_each_possible_cpu(cpu) {
		...
		if (boosted == 0) {
			if (old_freq != policy->cur) {
				...
				// This may also happen if we refused to ramp up because the nr_running_s()==1
				new_freq = old_freq;
				dprintk(SMARTASS_DEBUG_ALG,"smartassQ @ %d nothing: ramp_dir=%d nr_running_s=%lu\n",
		...
		cpufreq_notify_utilization_s(policy,
...
static int __init cpufreq_smartass_init(void)
{
	...
	boost_pulse = 0;
	SYMSEARCH_BIND_FUNCTION_TO(smartassH3, nr_running, nr_running_s);
