...
#include "symsearch.h"
SYMSEARCH_DECLARE_FUNCTION_STATIC(void, unlock_policy_rwsem_write_s, int);
SYMSEARCH_DECLARE_FUNCTION_STATIC(int, lock_policy_rwsem_write_s, int);
				    ...
				    const char *buf, size_t count)
{
	...
	if (!bypass) {
		if (reenable_timer) {
			...
			for_each_online_cpu(cpu) {
				if (lock_policy_rwsem_write_s(cpu) < 0)
					continue;
...
skip_this_cpu:
				unlock_policy_rwsem_write_s(cpu);
			}
		}
...
static int __init cpufreq_gov_dbs_init(void)
{
	...
	SYMSEARCH_BIND_FUNCTION_TO(intellimm, unlock_policy_rwsem_write, unlock_policy_rwsem_write_s);
	SYMSEARCH_BIND_FUNCTION_TO(intellimm, lock_policy_rwsem_write, lock_policy_rwsem_write_s);
	SYMSEARCH_BIND_FUNCTION_TO(intellimm,  sched_setscheduler_nocheck,  sched_setscheduler_nocheck_s);
	...
	for_each_possible_cpu(i) {
...
#endif
		{
			...
			if (likely(!IS_ERR(pthread))) {
				...
				sched_setscheduler_nocheck_s(pthread,
							SCHED_FIFO, &param);
